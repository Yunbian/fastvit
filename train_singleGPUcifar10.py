...
if __name__ == '__main__':
    ...
    dataset_train = datasets.CIFAR10("./cifar10", download=False, train=True, transform=transform)
    dataset_test = datasets.CIFAR10("./cifar10", download=False, train=False, transform=transform_test)
    ...
    test_loader = torch.utils.data.DataLoader(dataset_test, batch_size=BATCH_SIZE, pin_memory=True, shuffle=False,num_workers=1)
    ...
    # model_ft.to(DEVICE)
    model_ft.to(device)
    # model_ft = DistributedDataParallel(model_ft, device_ids=[device], output_device=device, find_unused_parameters=True)
    if use_ema:
        model_ema = ModelEma(
            model_ft,
