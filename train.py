...
def _parse_args():
    ...
    args_config, remaining = config_parser.parse_known_args()   #  args_config包含从命令行中指定的与配置相关的参数，而remaining包含剩余的未解析的参数。
    if args_config.config:  #确定是否有配置文件需要解析
        with open(args_config.config, "r") as f:
            cfg = yaml.safe_load(f)
            parser.set_defaults(**cfg)   #配置文件中的内容作为默认值设置给命令行参数解析器parser

    # The main arg parser parses the rest of the args, the usual
    # defaults will have been overridden if config file specified.
    args = parser.parse_args(remaining)   #解析剩余的命令行参数，并将解析后的结果保存在args变量中。

    # Cache the args as a text string to save them in the output dir later
    args_text = yaml.safe_dump(args.__dict__, default_flow_style=False)   #参数对象args转换成文本形式，并以YAML格式保存在args_text变量中。
    return args, args_text


def main():
    setup_default_logging()    #设置默认的日志记录配置
    ...
    if args.distributed:   #args.distributed设置为True，表示分布式计算环境；反之则设置为False，表示非分布式计算环境。
    ...
    if args.amp:
        ...
        if has_native_amp:    #当前环境是否支持原生AMP和APEX AMP
